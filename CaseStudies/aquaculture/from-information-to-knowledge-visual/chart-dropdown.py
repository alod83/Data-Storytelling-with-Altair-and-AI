...
# add the TREND column to the DataFrame with the following values:
# if YEAR_ID is less than 1992 then the value is 0 
# if YEAR_ID is less than 1998 then the value is 1
# if YEAR_ID is equal to 1998 then the value is 2
# if YEAR_ID is greater than 1998 then the value is 3

df['TREND'] = [0 if x < 1992 else 1 if x < 1998 else 2 if x == 1998 else 3 for x in df['YEAR_ID']]

#df['TREND'] = ['n.a.' if x < 1992 else 'negative' if x < 1998 else 'both' if x == 1998 else 'positive' for x in df['YEAR_ID']]
...
#input_dropdown = alt.binding_select(options=['positive', 'negative'])
#select_trend = alt.selection_point(name='Select', fields=['TREND'], bind=input_dropdown, value=[{'TREND': 'positive'}])

input_dropdown = alt.binding_select(options=[1, 3], labels=['negative', 'positive'])
select_trend = alt.selection_point(name='Select', fields=['TREND'], bind=input_dropdown, value=[{'TREND': 1}])
...
#chart = image
chart.save('chart-dropdown.html')